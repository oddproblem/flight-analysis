...
### Aviation Operations Research, Predictive Delay Forecasting & Root-Cause Platform
...
[**Explore Interactive App**](http://localhost:8501) · [**View ML Architecture**](#machine-learning-architecture--methodology) · [**STAR Resume Bullets**](#resume--interview-talking-points)
...
AeroPulse processes **469,968 real-world U.S. DOT flight records**, models the operational lifecycle through a PostgreSQL star schema, trains hierarchical forecasting models (Baselines vs. Histogram Gradient Boosting), and serves live predictions through an interactive executive command center.
...
│  - Baselines vs. HistGBM  │      │  - FAA Cost Multipliers   │      │  Dark Glassmorphic UI     │
│  - Permutation Drivers    │      │  - Dynamic Buffer Strategy│      │  Real-Time Simulator      │
└───────────────────────────┘      └───────────────────────────┘      └───────────────────────────┘
```
...
| **Tier 2 (Heuristic)** | **Route × Carrier Heuristic** | $\text{Median}(\text{Route} \times \text{Carrier})$ with carrier fallback | Industry operational rule-of-thumb |
...
| **Tier 3 (Risk Scoring)** | **HistGradientBoosting Classifier** | Log-loss risk classification ($\text{Delay} \ge 15\text{m}$) | Binary delay risk score & alert tier |

### 2. Out-of-Time Performance Comparison

#### Continuous Delay Duration Forecasting (Minutes)
| Model Architecture | MAE (min) | RMSE (min) | Median AE (min) | MAPE (%) | $R^2$ |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Naive Global Median** | 8.27 | 28.79 | 0.00 | 100.0% | -0.0898 |
| **Route × Carrier Heuristic** | 8.77 | 28.37 | 0.00 | 102.2% | -0.0577 |
| **HistGBM Delay Forecaster** | **16.56** | **29.09** | **11.75** | 226.4% | -0.1121 |

#### Binary Risk Classification ($\ge 15$ Minute Delay)
...
> In airline operations, arrival delay displays severe right-skewness: over 78% of flights arrive on-time ($\le 0$ min delay), while a tiny fraction suffer multi-hour delays due to airport ground stops or severe weather. Because RMSE squares individual deviations, extreme outlier delays inflate RMSE significantly over MAE. Monitoring **Median Absolute Error (11.75 min)** provides a much more faithful metric for daily gate scheduling.
...
AeroPulse includes an interactive dashboard with dark glassmorphic styling:

- **Tab 1: Executive KPI Command Center**: Top-level macro metrics (Volume, OTP-15, Average Delay, Total Financial Exposure, Cancellation Rate) paired with dual-axis delay cascade charts.
- **Tab 2: Live ML Delay Simulator**: Input any carrier, route, departure hour, and day of week to receive instantaneous delay probabilities, estimated duration, risk badge, and FAA cost impact.
- **Tab 3: Carrier & Airport Reliability Benchmarks**: Head-to-head airline ranking (Delta, Alaska, Southwest, United, American, etc.) and airport congestion rankings.
- **Tab 4: Model Diagnostics & Error Analysis**: Residual distribution charts, baseline comparison tables, and dollarized business recommendations.
- **Tab 5: SQL & Star Schema Intelligence**: Interactive warehouse view explorer showing production analytical SQL queries and live query results.
...
```bash
# 1. Clean raw Kaggle flight records into Parquet (takes ~10s)
python -m src.transform.clean_flight_data

# 2. Train and evaluate ML models (takes ~5s)
python -m src.ml.train

# 3. Generate business diagnostics
...
## 🗄️ Database Warehouse Architecture
